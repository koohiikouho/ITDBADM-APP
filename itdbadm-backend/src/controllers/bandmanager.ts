...
import { dbPool_manager as dbPool } from "../db";
...
export const bandManagerController = new Elysia({ prefix: "/band-manager" })
  ...
  .put(
    ...
    async ({ headers, set, jwt, body }) => {
      ...
      const userId = payload.id;
      const { band_id, name, genre, description, iframe_string, member_list } =
        body;
      ...
      try {
        ...
        await connection.execute("DELETE FROM band_members WHERE band_id = ?", [
          band_id,
        ]);
  ...
  .put("/band/pfp", async ({ headers, set, jwt, request }) => {
    const token = headers.authorization?.split(" ")[1];
    const payload = await jwt.verify(token);
  ...
  .post("/products", async ({ headers, set, jwt, request }) => {
    const token = headers.authorization?.split(" ")[1];
    const payload = await jwt.verify(token);
    ...
    try {
      ...
      if (imageUrls.length === 0) {
        imageUrls = [
          "https://res.cloudinary.com/dkuf0peg7/image/upload/v1763529474/band-6/products/kbc3gvzcnhbjxb39gohs.jpg",
        ];
      }
  ...
  .put("/products/:id", async ({ headers, set, jwt, params, request }) => {
    const token = headers.authorization?.split(" ")[1];
    const payload = await jwt.verify(token);
    ...
    try {
      ...
      bookingRows.forEach((row: any) => {
        activities.push({
          ...
          description: `Booking request for ${new Date(
            row.booking_date
          ).toLocaleDateString()}`,
  ...
  .get("/performance", async ({ headers, set, jwt }) => {
    ...
    try {
      ...
      const avgBookingPrice =
        parseFloat(bookingRows[0]?.avg_accepted_price) || 0;
      ...
      const topSellingProduct =
        topProductRows.length > 0 ? topProductRows[0]?.name : undefined;
